<template>
  <el-menu :max="1" :min="0" background-color="rgb(247, 244, 204)">
    <el-menu-item index="10"><el-button>Sort by</el-button></el-menu-item>
	...
	methods: {
		handleClick(index) {
          ...
          color: white;`
			...
			if (this.asideObj[index].clicked === false) {
				this.asideObj.forEach(x => {
					x.clicked = false
					x.style = ''
				})
				this.asideObj[index].clicked = true
				this.asideObj[index].style = styleObj
			} else if (this.asideObj[index].clicked === true) {
				this.asideObj.forEach(x => {
					x.clicked = false
				})
...
<style scoped>
.el-menu-item:first-child .el-button {
	width: 100%;
	padding: 0px;
	height: 100%;
...
.el-menu-item:not(:first-child) > .el-button {
	width: 100%;
	padding: 0px;
	height: 100%;
	background-color: rgb(247, 244, 204);
...
.el-menu-item:not(:first-child) > .el-button:hover {
	color: white;
	background-color: black;
	font-family: InputMonoCondensedItalic;
	border: 0px;
